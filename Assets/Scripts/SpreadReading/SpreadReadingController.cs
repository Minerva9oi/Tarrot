...
namespace Tarot.SpreadReading
{
    public sealed class SpreadReadingController : MonoBehaviour
    {
        ...
        private const float StagedParticleFlowDuration = 3.08f;
            ...
            float waitingScale)
        {
            ...
            while (elapsed < StagedParticleFlowDuration)
            {
                ...
                var cardFade = 1f - Smooth01(Mathf.InverseLerp(0.28f, 0.58f, progress));
        ...
        private void UpdateConvergeParticleBatch(ConvergeParticleBatch batch, float progress)
        {
            ...
            for (var index = 0; index < batch.Particles.Length; index++)
            {
                ...
                {
                    stream = Smooth01(Mathf.InverseLerp(0.42f, 1f, release));
                    ...
                    var gust = side * Mathf.Sin(stream * 13f + particle.FlowPhase) * 0.22f * Mathf.Sin(stream * Mathf.PI);
                ...
                var materialize = batch.StagedFlow
                    ...
                    : 1f;
                var fadeStart = batch.DissolveOnly ? 0.86f : 0.88f;
                var pale = batch.DissolveOnly ? Smooth01(Mathf.InverseLerp(0.72f, 1f, stream)) : 0f;
                color = Color.Lerp(color, Color.Lerp(color, Color.white, 0.18f), pale);
                color.a = Mathf.Lerp(batch.StagedFlow ? 1f : 0.98f, 0.08f, Smooth01(Mathf.InverseLerp(fadeStart, 1f, stream))) * materialize;
                var expansionPulse = batch.DissolveOnly ? 1f + Mathf.Sin(Mathf.Clamp01(stream / 0.34f) * Mathf.PI) * 0.28f : 1f;
                var scale = particle.Size * expansionPulse * Mathf.Lerp(batch.StagedFlow ? 1.95f : 1.24f, particle.EndScale, stream);
